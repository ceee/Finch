...
public class MixtapeMergedValidator<T> : IMixtapeMergedValidator<T>
{
  ...
  public MixtapeMergedValidator(IEnumerable<IValidator<T>> validators)
  {
  ...
  bool CanHandle(IValidator<T> validator, Type modelType)
  {
    ...
    Type typeToFind = typeof(MixtapeValidator<,>);
    ...
    Type mixtapeValidatorType = findValidatorBase(validatorType);

    if (mixtapeValidatorType == null)
    {
      return false;
    }

    Type implementationType = mixtapeValidatorType.GenericTypeArguments[1];
...
public interface IMixtapeMergedValidator<T>
{
  /// <summary>
  ...
  /// <summary>
  /// Validates a model by using all registered MixtapeValidators for this entity type
  /// </summary>
